...
# AIC.1.tmt has the lowest AIC

# Scatterplot of #Cones vs. Diameter
Col <- c(AMB = 'blue', CO2 = 'red')
plot(Dat$diam, Dat$cones, col = Col[Dat$tmt], 
     main = "Relationship between diameter of pine trees and the numers of cones",
     xlab = "Diameter", ylab = "number of cones")

# Vector of tree diameters for which #cones should be predicted
diam.vec <- seq(1,50,1)

# Model 1 - AMB
b0 <- MLE.1.tmt$par['b0']
b1 <- MLE.1.tmt$par['b1.AMB']
mu.pred <- b0 + b1 * log(diam.vec)
cones.1.AMB <- exp(mu.pred) 
lines(diam.vec, cones.1.AMB, col = Col['AMB'], lwd = 1.5)

# Model 1 - CO2
b0 <- MLE.1.tmt$par['b0']
b1 <- MLE.1.tmt$par['b1.CO2']
mu.pred <- b0 + b1 * log(diam.vec)
cones.1.CO2 <- exp(mu.pred) 
lines(diam.vec, cones.1.CO2, col = Col['CO2'], lwd = 1.5)

legend(5, 45, legend = c("CO2", "AMB"), box.lty = 0,
       col=c("red", "blue"), lty = 1, cex = 1, title="Treatments")
